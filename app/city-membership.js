...
import {newFarm,restoreFarm,farmAction} from './public/vitriny-farm-core.js';
export function setupCityMembership(app,{db,currentUser,requireUser,sameOriginOnly,isAdministrativeUser=()=>false}){
  ...
  app.post('/api/games/farm/action',sameOriginOnly,requireUser,(req,res)=>{
    ...
    const result=db.transaction(()=>{const result=farmAction(read(req.user.id),action,Date.now());if(result.changed)db.prepare(`INSERT INTO city_farm_progress(user_id,state_json) VALUES (?,?) ON CONFLICT(user_id) DO UPDATE SET state_json=excluded.state_json,updated_at=CURRENT_TIMESTAMP`).run(req.user.id,JSON.stringify(result.state));return result;})();
