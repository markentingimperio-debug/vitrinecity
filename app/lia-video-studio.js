  ...
  async function process(){
    ...
    try{
      ...
      if(due){
        ...
        try{const receipt=await publishVitrine(jobDto(readJob(due.id))),published=receipt?.status==='published',publicationId=String(receipt?.postId||receipt?.publicationId||'');db.prepare("UPDATE lia_video_jobs SET status=?,publication_id=?,error='',updated_at=CURRENT_TIMESTAMP WHERE id=?").run(published?'published':'ready',publicationId,due.id);db.prepare("UPDATE lia_video_distribution SET status=?,publication_id=?,publication_url=?,error='',updated_at=CURRENT_TIMESTAMP WHERE job_id=? AND provider='vitrine_social'").run(published?'published':String(receipt?.status||'processing'),publicationId,String(receipt?.publicUrl||''),due.id);}
