...
export function ecosystemProviderIssue(value){const text=String(value||'');return /inference is blocked on this account|account.{0,30}(?:blocked|suspended|disabled)|Geração bloqueada na conta do provedor/i.test(text)?{code:'provider_account_block',detail:'A conta do provedor está com a geração bloqueada. Confira a situação da conta no serviço.'}:/zdr|data.policy|data.collection|privacy|no endpoints found matching|política de dados atual/i.test(text)?{code:'provider_data_policy',detail:'O provedor bloqueou a geração por política de dados. A configuração precisa ser revisada; nenhuma proteção foi alterada.'}:/insufficient.{0,30}credits|key limit exceeded|\b402\b|Provedor recusou por saldo ou limite/i.test(text)?{code:'provider_balance',detail:'O provedor informou limite ou saldo insuficiente. Confira a conta antes de repetir a geração.'}:/unauthorized|invalid.{0,20}key|\b401\b/i.test(text)?{code:'provider_access',detail:'O provedor recusou a autorização. Confira a conexão da conta.'}:/rate.limit|\b429\b/i.test(text)?{code:'provider_rate_limit',detail:'O provedor aplicou um limite temporário de solicitações.'}:{code:'provider_generation_failed',detail:'A geração não foi concluída. Confira a tarefa no estúdio antes de tentar novamente.'};}
...
export function createEcosystemOrchestrator({db,getStories,catalog,runInternalSocial=async()=>({}),getInternalSocial=()=>({}),now=Date.now,schedule=true}){
  ...
  function snapshot(){
    ...
    if(modules.videos.scenes?.failed){
      const grouped=new Map();
      for(const row of db.prepare("SELECT error_message,COUNT(*) count FROM viral_quiz_scenes WHERE status='failed' GROUP BY error_message").all()){
        const issue=ecosystemProviderIssue(row.error_message),prior=grouped.get(issue.code);
        grouped.set(issue.code,{...issue,count:Number(row.count)+(prior?.count||0)});
      }
