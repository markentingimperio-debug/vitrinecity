...
export function createStoryImageProvider({request,model,outputDir,provider='openrouter',format='portrait',prefix='story-ai'}) {
  if(!['portrait','landscape'].includes(format)||!['story-ai','editorial-ai'].includes(prefix))throw requestError('story_image_configuration_invalid',400);
  return async prompt=>{
    ...
    const body={model:actualModel,prompt:String(prompt).slice(0,1600),n:1,...(selected==='openai'?{size:format==='landscape'?'1536x1024':'1024x1536',quality:'medium'}:{aspect_ratio:format==='landscape'?'16:9':'9:16'})};
    const result=await request(selected==='openai'?OPENAI_IMAGE_URL:'https://openrouter.ai/api/v1/images',{method:'POST',body:JSON.stringify(body),redirect:'error'},120000);
    ...
    if(bytes.length>8*1024*1024||Math.min(size.width,size.height)<640||size.width>10000||size.height>10000||size.width*size.height>40000000||(format==='landscape'?size.width<=size.height:size.height<=size.width))throw Error('story_image_quality');
    const file=prefix+'-'+randomUUID()+'.'+(size.type==='jpeg'?'jpg':size.type);
