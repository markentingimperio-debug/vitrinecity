...
const commentScopes=['pages_messaging','pages_manage_metadata','instagram_manage_comments'];
...
export function verifySocialOauthState(value,userId,{secret,isAdmin=false,now=Date.now()}={}){
  ...
  try{const data=JSON.parse(Buffer.from(payload,'base64url').toString('utf8'));data.intent??='read_only';
    ...
    if(!isAdmin&&(data.intent==='comment_replies'||data.returnTo!=='carteira'))return null;
    return data;
  }catch{return null;}
}
