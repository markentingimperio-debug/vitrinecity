...
export function canRunOperation(value){return !!value&&value.policy.enabled&&!value.policy.paused&&value.automation.enabled===true&&value.automation.configured===true&&value.automation.running===false&&!value.automation.closed&&(number(value.automation.quota?.remaining)>0||value.policy.internalSocialEnabled===true);}
...
export function mountOperations(root,{document=root.ownerDocument,window=document.defaultView,fetcher=(...args)=>fetch(...args),setTimer=setTimeout,clearTimer=clearTimeout,pollMs=30000}={}){
  ...
  function renderHeader(){
    ...
    const state=p.paused?'Pausa global ativa':a.running===true?'Preparação em andamento':!p.enabled?'Coordenação desligada':a.configured!==true?'Aguardando configuração':a.enabled!==true?'Produção diária desligada':number(a.quota?.remaining)===0?'Limite de criação utilizado':'Rotina programada';
  ...
  function renderPlan(){
    ...
    for(const item of items.slice(0,50)){const li=node('li'),time=node('time',timeLabel(item.startedAt||item.finishedAt)),copy=node('div',undefined,'item-copy'),heading=node('div',undefined,'item-head');heading.append(node('h3',item.label||'Etapa da operação'),badge(item.status));copy.append(heading);if(item.reason)copy.append(node('p',item.reason));const open=link('Abrir detalhe ↗',item.url);if(open)copy.append(open);li.append(time,copy);target.append(li);}
    ...
    STEPS.forEach((step,index)=>{const rows=items.filter(item=>step.kinds.includes(item.kind)),li=node('li');li.append(node('span',String(index+1).padStart(2,'0'),'step-index'),node('strong',step.label));let description='Sem atividade informada';if(rows.length){const working=rows.filter(item=>['running','in_progress','processing'].includes(item.status)).length,needs=rows.filter(item=>['review','blocked','failed','interrupted'].includes(item.status)).length;description=working?working+' em andamento':needs?needs+' precisam de atenção':rows.length+' etapas registradas';}li.append(node('small',description));flow.append(li);});
  }
  ...
  async function responseData(response){let data;try{data=await response.json();}catch{throw Error('O painel recebeu uma resposta incompleta. Atualize novamente.');}if(response.status===401||response.status===403){const error=Error('Seu acesso expirou ou não permite esta operação. Entre novamente pelo painel administrativo.');error.status=response.status;throw error;}if(!response.ok){const error=Error(response.status===409?'A rotina mudou em outra sessão. Atualize antes de salvar novamente.':text(data?.error)||'Não foi possível concluir. Atualize o painel e confira o estado.');error.status=response.status;throw error;}return data;}
