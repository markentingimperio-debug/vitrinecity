import {createResidentCatalog,residentConversation,STUDIO_CHANNELS} from './vitriny-city-residents-core.js?v=20260915-residents-2';
...
export function installCityResidents({document:doc=document,events=window}={}){
  ...
  const personLabel=element('label','Pessoa virtual ou especialidade'),personSelect=element('select');personSelect.id='residentsPerson';personLabel.htmlFor=personSelect.id;
  ...
  const description=element('p',null,'resident-description'),routine=element('p',null,'resident-routine');
  ...
  const exchange=element('details',null,'resident-exchange'),exchangeText=element('p',null,'resident-speech');exchange.append(element('summary','Troca de ideias entre departamentos · simulação'),exchangeText);
  ...
  dialog.append(header,notice,filters,profile,description,status,actions,routine,speech,exchange,channels,admin,footer);doc.body.append(dialog);
  ...
  function render(){
    ...
    routine.textContent=d.newBuilding&&p.specialty?'Rotina simulada: mesa de trabalho → saída pela entrada → conversa com colega → retorno à mesa.':'Rotina simulada na recepção aberta: orientação → deslocamento curto → conversa. Não há interior de trabalho conectado neste prédio.';
    exchangeText.textContent=residentConversation(p,catalog,{crossDepartment:true}).lines.join('\n');exchange.open=false;
    ...
    footer.textContent=`${catalog.residents.length} cadastros virtuais · não são executores nem pessoas online. ${sceneReady?`Até ${lite?16:40} exibidos em 3D por vez; selecionar uma pessoa prioriza os colegas do seu departamento.`:'O catálogo funciona mesmo sem a navegação 3D.'}`;
  }
  ...
  chat.addEventListener('click',()=>{const p=catalog.residents.find(p=>p.id===selectedId),conversation=residentConversation(p,catalog);speech.textContent=`Conversa simulada · roteiro\n${conversation.lines.join('\n')}`;speech.hidden=false;chat.setAttribute('aria-expanded','true');});
