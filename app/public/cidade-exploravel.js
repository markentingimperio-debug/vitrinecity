(() => {
  ...
  function constrain() {
    ...
    offsetX = Math.min(
      34,
      Math.max(viewport.clientWidth - width - 34, offsetX),
    );
    offsetY = Math.min(
      34,
      Math.max(viewport.clientHeight - height - 34, offsetY),
    );
  }
  function draw() {
    constrain();
  ...
  function center() {
    scale = Math.max(
      ...
      Math.min(
        0.72,
        Math.min(
          viewport.clientWidth / WORLD_WIDTH,
          viewport.clientHeight / WORLD_HEIGHT,
        ) * 1.25,
      ),
    );
  ...
  function zoom(
    ...
    scale = Math.max(0.18, Math.min(1.65, scale + delta));
  ...
  function pinKey(pin) {
    return (
      pin.dataset.key ||
      (pin.dataset.name || pin.textContent.trim())
  ...
  function updateFavoriteUi() {
    ...
    favoriteAction.textContent = saved
      ? "♥ Salvo nos favoritos"
  ...
  function focusOnPin(pin, openPanel = true) {
    if (!pin) return;
    scale = Math.max(scale, viewport.clientWidth < 760 ? 0.62 : 0.78);
  ...
  function showPanel(pin) {
    ...
    const lotAvailable =
      isLot &&
      (!pin.dataset.lotStatus || pin.dataset.lotStatus === "available");
    setAction(
      "primaryAction",
      lotAvailable
        ? `/comprar-lote.html?lote=${encodeURIComponent(pin.dataset.lot)}`
        : "",
      lotAvailable,
      "Escolher este prédio",
    );
    setAction(
      "pageAction",
      pin.dataset.page,
      Boolean(pin.dataset.page),
    ...
    setAction(
      "instagramAction",
      pin.dataset.instagram,
      Boolean(pin.dataset.instagram),
    );
    updateFavoriteUi();
  ...
  viewport.addEventListener(
    ...
    (event) => {
      ...
      zoom((distance - pinchDistance) / 260);
      pinchDistance = distance;
    },
    { passive: false },
  );

  const serviceAds = [...document.querySelectorAll(".city-service-ad")];
  const money = (value) =>
      ...
      const renderAds = () =>
        serviceAds.forEach((ad, index) => {
          ...
          const priceText = service.amountCents
            ...
            : service.label;
          ad.dataset.tag = service.label || "PUBLICIDADE";
  ...
  document.getElementById("zoomIn").onclick = () => zoom(0.14);
  document.getElementById("zoomOut").onclick = () => zoom(-0.14);
  ...
  function filterPins() {
    ...
    pins.forEach((pin) => {
      const text =
        `${pin.dataset.name || ""} ${pin.dataset.desc || ""} ${pin.dataset.tag || ""}`.toLocaleLowerCase(
          "pt-BR",
        );
      const matchesFilter =
        activeFilter === "all" ||
        pin.dataset.category === activeFilter ||
        (activeFilter === "favorites" && favorites.has(pinKey(pin)));
  ...
  if (window.matchMedia("(max-width: 760px)").matches)
    cityPulse.classList.add("collapsed");
  pulseToggle.setAttribute(
    "aria-expanded",
    String(!cityPulse.classList.contains("collapsed")),
  );
  pulseToggle.onclick = () => {
  ...
  dockButtons.forEach(
    (button) =>
      (button.onclick = () => {
        ...
        if (action === "courses")
          focusOnPin(
            pins.find((item) => pinKey(item) === "centro-educacional"),
          );
  ...
  document.getElementById("avatarBtn").onclick = () =>
    avatarShop.classList.add("show");
  document.getElementById("closeAvatar").onclick = () =>
    avatarShop.classList.remove("show");
  avatarShop.addEventListener("click", (event) => {
    if (event.target === avatarShop) avatarShop.classList.remove("show");
  });
  ...
  syncLots().finally(() => {
    ...
    if (requestedLot)
      requestAnimationFrame(() =>
        focusOnPin(pins.find((item) => item.dataset.lot === requestedLot)),
      );
  });
})();
