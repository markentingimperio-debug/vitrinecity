...
export function createSearchReader({document, origin, getRecommendations}) {
  ...
  function renderRecommendations(current) {
    ...
    for(const item of getRecommendations()) {
      ...
      row.append(link(item.title,item.url,item.affiliate),el('small',item.affiliate?'Oferta de afiliado · Podemos receber comissão':'Conteúdo ou negócio da Vitrine'));
    ...
    channel.append(el('h3','Da nossa equipe'),link('Agrotécnica no YouTube ↗','https://www.youtube.com/@agrotecnica362'),el('p','Canal indicado pela equipe VitrineCity. Abre no YouTube, em outra aba.','reader-note'));
