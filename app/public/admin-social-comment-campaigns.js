...
export function suggestedCaption(source,keyword='EU QUERO') {
  ...
  return [title,summary,`Quer acessar o conteúdo completo? Comente ${term} para receber o link por mensagem privada.`,source?.commercial?'Publicidade.':''].filter(Boolean).join('\n\n');
}

export function socialCampaignPayload(values) {
  const {sourceKey,accountId,surface,postId='',groupId='',keyword,caption,invite='none'}=values;
  ...
  if(!caption.toUpperCase().includes(keyword))throw Error('Inclua na descrição a frase que a pessoa deve comentar: '+keyword+'.');
  return {sourceKey,accountId,surface,postId,groupId:surface==='facebook_group'?groupId:'',keyword,caption:caption.trim(),invite};
}
...
export function mountSocialCampaigns(root,{document=root?.ownerDocument,window=document?.defaultView,fetcher=(...args)=>fetch(...args),randomUUID=()=>crypto.randomUUID(),setTimer=setTimeout,clearTimer=clearTimeout}={}) {
  ...
  const ids=['Form','Notice','Reload','Search','SearchCatalog','Source','SourceCount','SourceCard','Account','Surface','CheckConnection','ConnectionState','ConnectionTitle','ConnectionMissing','ConnectionChecks','PostId','GroupId','GroupField','Keyword','Caption','Suggest','Invite','Prepare','Preview','PreviewTitle','Summary','Readiness','Missing','Cover','PublicCaption','PrivateReply','Copy','Counts','PreviewNote','Activate','Pause','Refresh','History'];
  ...
  function validCampaign(value){const data=value?.campaign||value;if(!data||!idPattern.test(String(data.id||''))||!Object.hasOwn(statuses,data.status)||!data.source?.title||!data.account||!Object.hasOwn(surfaces,data.surface)||!data.readiness||!Array.isArray(data.readiness.missing)||typeof data.caption!=='string'||typeof data.privateReply!=='string')throw Error('Não foi possível ler a prévia completa. Atualize o andamento.');return data;}
  function renderCampaign(data){
    campaign=data;ui.Preview.hidden=false;ui.PreviewTitle.textContent=statuses[data.status];
    ui.Summary.textContent=[data.source.title,surfaces[data.surface],data.account.pageName||data.account.instagramUsername||'Conta da empresa',data.postId?'Publicação: '+data.postId:'Publicação ainda não vinculada','Pedido: '+data.keyword].join(' · ');
    ...
    const c=data.counts||{};ui.Counts.textContent='Aceitas pelo serviço: '+(Number(c.sent)||0)+' · Pendentes: '+(Number(c.pending)||0)+' · Em envio: '+(Number(c.processing)||0)+' · Falhas: '+(Number(c.failed)||0)+' · Sem confirmação: '+(Number(c.unknown)||0)+' · Canceladas: '+(Number(c.cancelled)||0);
    ui.PreviewNote.textContent=stale?'O formulário mudou. Prepare uma nova prévia antes de ativar respostas.':data.status==='active'?'Respostas ativas apenas para novos comentários que peçam o link desta publicação. As contagens não comprovam leitura ou compra.':data.status==='paused'?'Respostas pausadas. Uma mensagem já em envio pode concluir.':'Copie a descrição e publique a foto no canal escolhido. Depois informe o ID da publicação no formulário e prepare uma nova prévia. Ativar respostas autoriza a mensagem privada mostrada acima.';
  ...
  async function checkConnection(){
    ...
    try{const query='?accountId='+encodeURIComponent(ui.Account.value)+'&surface='+encodeURIComponent(ui.Surface.value),data=await request('/connection'+query);if(destroyed)return;
  ...
  listen(ui.Source,'change',()=>{sourceCard();changed();});listen(ui.Suggest,'click',()=>{const source=selectedSource();if(source){ui.Caption.value=suggestedCaption(source,ui.Keyword.value);changed();notice('Sugestão aplicada. Revise o texto antes de preparar a prévia.');}});
  for(const id of ['Account','Surface'])listen(ui[id],'change',()=>{ui.ConnectionState.hidden=true;changed();});
