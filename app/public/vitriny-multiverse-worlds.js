...
function addPortal(city,index,total){const angle=index/total*Math.PI*2-Math.PI/2,r=38,x=Math.cos(angle)*r,z=Math.sin(angle)*r,color=palette[index%palette.length],group=new THREE.Group();group.position.set(x,0,z);group.rotation.y=-angle+Math.PI/2;group.userData={portal:true,city};
  const frameMat=new THREE.MeshStandardMaterial({color:0x16263a,metalness:.7,roughness:.24}),glowMat=new THREE.MeshBasicMaterial({color,transparent:true,opacity:city.status==='active'?.78:.38});
...
renderer.domElement.addEventListener('pointerup',e=>{const click=Math.hypot(e.clientX-startX,e.clientY-startY)<6;dragging=false;if(!click)return;const portal=portalAt(e);if(portal?.userData.city.status==='active')location.assign('/vitriny-multiverse-explore.html');else if(portal)show(portal);});
