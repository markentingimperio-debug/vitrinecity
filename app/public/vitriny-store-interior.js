...
import {createStoreReception,storeReceptionPose} from '/vitriny-store-reception-core.js?v=20260915-reception-1';
...
const receptionButton=document.getElementById('openReception'),receptionDialog=document.getElementById('receptionDialog'),receptionClose=document.getElementById('closeReception'),motionButton=document.getElementById('pauseReception');
const reducedMotion=matchMedia('(prefers-reduced-motion: reduce)');
let reception=null,receptionCrowd=null,receptionRoot=null,receptionElapsed=0,receptionPosed=false,motionPaused=false;
...
function openReception(){
  if(disposed||!reception||receptionDialog.open)return;
  keys.clear();dragging=false;receptionDialog.showModal();receptionClose.focus();
}
function closeReception(){if(receptionDialog.open)receptionDialog.close();}
function restoreReceptionFocus(){if(!disposed)receptionButton.focus();}
function updateMotionButton(){const paused=motionPaused||reducedMotion.matches;motionButton.textContent=reducedMotion.matches?'Movimento reduzido':paused?'Retomar movimento':'Pausar movimento';motionButton.setAttribute('aria-pressed',String(paused));motionButton.disabled=reducedMotion.matches;}
function onReducedMotion(){updateMotionButton();}
receptionButton.addEventListener('click',openReception);
receptionClose.addEventListener('click',closeReception);
receptionDialog.addEventListener('close',restoreReceptionFocus);
receptionDialog.addEventListener('cancel',event=>{event.preventDefault();closeReception();});
receptionDialog.addEventListener('keydown',event=>{
  if(event.key!=='Tab')return;
  const controls=[...receptionDialog.querySelectorAll('a[href],button:not([disabled])')],first=controls[0],last=controls.at(-1);
  if(event.shiftKey&&document.activeElement===first){event.preventDefault();last?.focus();}
  else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first?.focus();}
});
motionButton.addEventListener('click',()=>{motionPaused=!motionPaused;updateMotionButton();});
reducedMotion.addEventListener('change',onReducedMotion);updateMotionButton();

function tickReception(dt){
  if(!receptionCrowd)return;
  const paused=motionPaused||reducedMotion.matches||document.hidden||receptionDialog.open;
  if(paused&&receptionPosed)return;
  if(!paused)receptionElapsed+=Math.min(.05,Math.max(0,dt));
  const pose=storeReceptionPose(),person=receptionCrowd.people[0];
  person.group.position.set(pose.x,pose.y,pose.z);person.group.rotation.y=pose.yaw;
  person.pose(receptionElapsed*3,pose.moving,pose.action);receptionCrowd.update();receptionPosed=true;
}
function mountReception(store){
  reception=createStoreReception(store);if(!reception||disposed)return;
  receptionRoot=new THREE.Group();receptionRoot.name='store-virtual-reception';scene.add(receptionRoot);
  receptionCrowd=createUrbanCrowd({parent:receptionRoot,count:1,identities:[reception.identity],profileId:profile.id});
  receptionCrowd.people[0].group.name=`resident-${reception.identity.id}`;
  receptionCrowd.people[0].group.userData={residentId:reception.identity.id,simulated:true};
  tickReception(0);
  document.getElementById('receptionName').textContent=reception.identity.name;
  document.getElementById('receptionGreeting').textContent=reception.greeting;
  document.getElementById('receptionNotice').textContent=reception.notice;
  document.getElementById('receptionAvailability').textContent=reception.availability;
  document.getElementById('receptionStore').href=reception.storeHref;
  document.getElementById('receptionLia').href=reception.liaHref;
  receptionButton.hidden=false;motionButton.hidden=false;
}
...
function disposeScene(event){keys.clear();dragging=false;if(event.persisted||disposed)return;disposed=true;cancelAnimationFrame(raf);closeReception();reducedMotion.removeEventListener('change',onReducedMotion);receptionCrowd?.dispose();receptionCrowd=null;if(receptionRoot)scene.remove(receptionRoot);scene.traverse(obj=>{obj.geometry?.dispose?.();if(obj.material){for(const material of Array.isArray(obj.material)?obj.material:[obj.material]){material.map?.dispose?.();material.dispose?.();}}});architecture.dispose();disposeReflections();renderer.dispose();}
...
addEventListener('keydown',event=>{if(receptionDialog.open||event.target?.closest?.('a,button,input,textarea,select,[contenteditable="true"]'))return;setKey(event.code,true);if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space'].includes(event.code))event.preventDefault();});addEventListener('keyup',event=>setKey(event.code,false));
...
const pointer=new THREE.Vector2(),raycaster=new THREE.Raycaster(),receptionHit=new THREE.Vector3();
const receptionPose=storeReceptionPose(),receptionBounds=new THREE.Box3(new THREE.Vector3(receptionPose.x-.65,0,receptionPose.z-.65),new THREE.Vector3(receptionPose.x+.65,2.5,receptionPose.z+.65));
function targetAtPointer(event){pointer.x=event.clientX/innerWidth*2-1;pointer.y=-(event.clientY/innerHeight)*2+1;raycaster.setFromCamera(pointer,camera);const hit=raycaster.intersectObjects(productTargets,true)[0];
  if(reception&&raycaster.ray.intersectBox(receptionBounds,receptionHit)&&(!hit||raycaster.ray.origin.distanceTo(receptionHit)<hit.distance))return {reception:true};
  let node=hit?.object||null;while(node&&!node.userData?.product)node=node.parent;return node?.userData?.product?node:null;
}
...
renderer.domElement.addEventListener('pointerup',event=>{const click=Math.hypot(event.clientX-pointerStartX,event.clientY-pointerStartY)<6;dragging=false;if(click){const target=targetAtPointer(event);if(target?.reception)openReception();else if(target)safeNavigate(target.userData.href);}});
renderer.domElement.addEventListener('pointermove',event=>{if(dragging){yaw+=(event.clientX-lastX)*.0042;pitch=Math.max(-.7,Math.min(.5,pitch-(event.clientY-lastY)*.0034));lastX=event.clientX;lastY=event.clientY;}else{const target=targetAtPointer(event);focusProduct.textContent=target?.reception?`${reception.identity.name} · Recepção virtual`:target?`${target.userData.label} · ${(target.userData.priceCents/100).toLocaleString('pt-BR',{style:'currency',currency:'BRL'})}`:'Clique em um produto para abrir os detalhes.';renderer.domElement.style.cursor=target?'pointer':'grab';}});
...
function animate(now){if(disposed)return;raf=requestAnimationFrame(animate);const dt=Math.min(.05,(now-last)/1000);last=now;if(document.hidden)return;
  ...
  if(receptionDialog.open)velocity.set(0,0,0);if(velocity.lengthSq())velocity.normalize().multiplyScalar(speed*dt);position.add(velocity);position.x=Math.max(-room.width/2+1.4,Math.min(room.width/2-1.4,position.x));position.z=Math.max(-room.depth+10,Math.min(14,position.z));
  const look=new THREE.Vector3(Math.sin(-yaw)*Math.cos(pitch),Math.sin(pitch),Math.cos(-yaw)*Math.cos(pitch));camera.position.copy(position);camera.lookAt(position.clone().add(look));tickReception(dt);renderer.render(scene,camera);
}
