...
export function registerSocialCommentCampaigns({app,db,requireAdmin,sameOriginOnly,siteUrl,sourceCatalog,metaAdapter,now=Date.now,sendTimeoutMs=20000,inspectTimeoutMs=35000}) {
  ...
  function reply(source,invite,id,surface){
    ...
    return `Aqui está o conteúdo que você pediu: ${source.title}\n${url.href}${disclosure}${invitation}`;
  }
  ...
  async function performInspection(data){
    ...
    try{return readinessResult(await Promise.race([metaAdapter.inspect({accountId:data.accountId,surface:data.surface,postId:data.postId||'',groupId:data.groupId||''}),new Promise((_,reject)=>{timer=setTimeout(()=>reject(Error('inspection_timeout')),Math.max(1,Math.min(inspectTimeoutMs,50000)));})]));}
  ...
  function dto(row){
    ...
    return {id:row.id,status:row.status,source:JSON.parse(row.source_json),account:JSON.parse(row.account_json),surface:row.surface,postId:row.post_id,groupId:row.group_id,keyword:row.keyword,caption:row.caption,invite:row.invite,privateReply:row.private_reply,readiness:JSON.parse(row.readiness_json),counts,createdAt:new Date(row.created_at).toISOString(),activatedAt:row.activated_at?new Date(row.activated_at).toISOString():null};
  }
  function validateInput(input){
    ...
    return {sourceKey,accountId,surface,postId,groupId,keyword,caption,invite,idempotencyKey,account:publicAccount(account),objectId};
  }
  ...
  app.get(API+'/connection',requireAdmin,route(async(req,res)=>{
    ...
    const key=hash([accountId,surface,account]);let pending=connectionChecks.get(key);
    if(!pending){if(connectionChecks.size>=50)throw fail('Aguarde um instante antes de verificar outra conexão.',429);pending={expiresAt:now()+5000,promise:performInspection({accountId,surface})};connectionChecks.set(key,pending);}
  ...
  app.post(API+'/preview',requireAdmin,sameOriginOnly,route(async(req,res)=>{
    const data=validateInput(req.body),requestHash=hash([data.sourceKey,data.accountId,data.surface,data.postId,data.groupId,data.keyword,data.caption,data.invite]);
    ...
    const promise=(async()=>{
      ...
      const caption=data.caption||`${withoutLinks(snapshot.title)}\n\n${withoutLinks(snapshot.summary.slice(0,650))}\n\nQuer acessar o conteúdo completo? Comente ${data.keyword} para receber o link por mensagem.`,privateReply=reply(snapshot,data.invite,id,data.surface);
      if(privateReply.length>1900)throw fail('O endereço e o título deste conteúdo excedem o tamanho permitido na mensagem.',409);
      db.prepare(`INSERT INTO social_content_campaigns VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(id,data.idempotencyKey,requestHash,'draft',data.sourceKey,JSON.stringify(snapshot),fingerprint,data.accountId,JSON.stringify(data.account),data.objectId,data.surface,data.postId,data.groupId,data.keyword,caption,data.invite,privateReply,JSON.stringify(readiness),created,null,created);
      return rowById(id);
  ...
  app.post(API+'/:id/activate',requireAdmin,sameOriginOnly,route(async(req,res)=>{
    ...
    const readiness=await inspect({accountId:row.account_id,surface:row.surface,postId:row.post_id,groupId:row.group_id},current.snapshot);
  ...
  function ingestWebhook(payload){
    ...
    db.transaction(()=>{for(const event of parseEvents(payload)){
      ...
      else if(!requested(event.text,campaign.keyword))reason='Comentário sem a frase explícita escolhida.';
      else if(!accountById(campaign.account_id))reason='Conta desconectada.';
      else {try{if(sourceSnapshot(campaign.source_key).fingerprint!==campaign.source_hash)reason='O conteúdo foi alterado.';}catch{reason='Conteúdo indisponível.';}}
      ...
      db.prepare(`INSERT OR IGNORE INTO social_content_comment_events (id,campaign_id,surface,object_id,page_id,instagram_id,post_id,group_id,comment_id,author_id,original_time,received_at,intent_text,status,reason) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(randomUUID(),campaign.id,event.surface,event.objectId,account.pageId,account.instagramId,event.postId,event.groupId,event.commentId,author,validTime?event.time:0,time,reason?'':normalize(event.text).slice(0,100),reason?'ignored':'pending',reason||null);
    }})();return claimed;
  }
  function claim(id){
  ...
  function freshCampaign(event){
    ...
    if(!currentEvent||currentEvent.status!=='processing'||currentEvent.invalidated_at!==null)throw fail('Este pedido foi invalidado antes do envio.',409);
  ...
  async function processPending(){
    ...
    if(!openHours(now()))return {processed:0};processing=true;let processed=0;
    try{for(const next of db.prepare("SELECT id FROM social_content_comment_events WHERE status='pending' ORDER BY received_at,id LIMIT 3").all()){
      const event=claim(next.id);if(!event)continue;processed++;
      let campaign;
      try{
        campaign=freshCampaign(event);const readiness=await inspect({accountId:campaign.account_id,surface:campaign.surface,postId:campaign.post_id,groupId:campaign.group_id},JSON.parse(campaign.source_json));
      ...
      }catch{db.prepare("UPDATE social_content_comment_events SET status='cancelled',reason='Pedido cancelado na verificação anterior ao envio.' WHERE id=? AND status='processing'").run(event.id);continue;}
      let timer;
      try{
        const result=await Promise.race([Promise.resolve().then(()=>metaAdapter.send({accountId:campaign.account_id,surface:campaign.surface,commentId:event.comment_id,text:campaign.private_reply})),new Promise((_,reject)=>{timer=setTimeout(()=>reject(Error('send_timeout')),Math.max(1,Math.min(sendTimeoutMs,30000)));})]);
        if(typeof result?.messageId!=='string'||!result.messageId.trim()||result.messageId.length>500||/[\x00-\x1f\x7f]/.test(result.messageId))throw Error('missing_provider_confirmation');
      ...
      }catch(error){db.prepare("UPDATE social_content_comment_events SET status=?,reason=? WHERE id=? AND status='processing'").run(error?.definitive===true?'failed':'unknown',error?.definitive===true?'O serviço recusou o envio. Nenhuma repetição automática será feita.':'Confirmação de envio desconhecida. Confira a conversa antes de qualquer novo envio.',event.id);}
      finally{clearTimeout(timer);}
    }}finally{processing=false;}return {processed};
  }
