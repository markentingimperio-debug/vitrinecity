...
export function createWebStoryAI({requestText,requestImage,assets,siteUrl='https://vitrinecity.com',dataDir,catalogImageFetcher=fetchCatalogImage}) {
  ...
  async function generate(source,{signal,isCurrent=()=>true,buttons={}}={}) {
    ...
    try {
      ...
      await checkpoint();const parsed=parse(raw);if(parsed.insufficient===true)return hold('source_insufficient_for_ten_pages');copy=validateCopy(normalizeStoryCopy(parsed,{affiliate,homeCta:!!homeCta}),sourceText,{affiliate,homeCta:!!homeCta,companion:source.kind==='trend'});
