...
import {assertCoinStatus,atomsFromRewardPoints,VITRINE_COINS_POLICY} from './public/vitrine-coins-contract.js';
const DAY=86400000;
...
export function setupCityRewards({app,db,coinWallet=null,expectedCollectorId=process.env.VITRINY_NEURAL_AI_MP_COLLECTOR_ID,requireUser,requireAdmin,sameOriginOnly,publicDir,getCourse,createPreference,paymentReady,searchPayments=null,onSettlement=()=>{},affiliateFor=()=>null,now=Date.now}){
  ...
  const unified=()=>coinWallet?.enabled===true;
  const paymentConfigured=()=>paymentReady()&&(!unified()||/^\d{1,30}$/.test(String(expectedCollectorId||'')));
  if(unified()&&db.prepare('SELECT coins_per_real rate FROM city_reward_settings WHERE id=1').get().rate!==100)throw Error('coin_rewards_conversion_review_required');
  const settings=()=>{const config=db.prepare('SELECT coins_per_real coinsPerReal,daily_limit dailyLimit,enabled FROM city_reward_settings WHERE id=1').get();return unified()?{...config,coinsPerReal:100,unified:true,pointsUnit:'legacy_cent_brl',coinPolicyVersion:VITRINE_COINS_POLICY.version}:config;};
  const available=id=>{
    if(unified()){const coins=assertCoinStatus(coinWallet.status(id));return {points:Number(BigInt(coins.availableAtoms)/960000n),nextExpiry:null,coins};}
    return db.prepare('SELECT COALESCE(SUM(remaining),0) points,MIN(expires_ms) nextExpiry FROM city_reward_batches WHERE user_id=? AND remaining>0 AND expires_ms>?').get(id,now());
  };
  ...
  function debit(id,points,reference,kind='course'){
    if(unified()){
      if(!points)return [];
      const amountAtoms=atomsFromRewardPoints(points,100),requestId=`reward:${reference}`;
      coinWallet.reserve(id,{requestId,maximumAtoms:amountAtoms,quoteId:reference,
        requestHash:createHash('sha256').update(JSON.stringify({id,points,reference,kind})).digest('hex'),service:kind==='avatar'?'reward_avatar':'reward_course'});
      return [{coinRequestId:requestId,amountAtoms}];
    }
    let remaining=points;const allocations=[];for(const batch of db.prepare('SELECT id,remaining FROM city_reward_batches WHERE user_id=? AND remaining>0 AND expires_ms>? ORDER BY expires_ms,id').all(id,now())){if(!remaining)break;const used=Math.min(remaining,batch.remaining);db.prepare('UPDATE city_reward_batches SET remaining=remaining-? WHERE id=?').run(used,batch.id);allocations.push({id:batch.id,points:used});remaining-=used;}if(remaining)throw Error('As moedas disponíveis mudaram. Confira novamente o valor.');return allocations;
  }
  function restore(order){
    if(!order.debited)return;
    for(const item of JSON.parse(order.allocations)){
      if(item.coinRequestId){
        if(!unified())throw Error('A carteira deste pedido precisa de conferência.');
        if(order.status==='approved')coinWallet.restore(order.user_id,item.coinRequestId,{reason:'reward_payment_reversed'});
        else coinWallet.release(order.user_id,item.coinRequestId,{reason:'reward_payment_not_consumed',noConsumptionConfirmed:true});
      }else if(unified()){
        // Preserve an old confirmed order's right to reversal without reviving
        // its now read-only legacy balance or extending the original expiry.
        const batch=db.prepare('SELECT * FROM city_reward_batches WHERE id=? AND user_id=?').get(item.id,order.user_id);
        if(!batch||!Number.isSafeInteger(item.points)||item.points<0)throw Error('Reserva antiga precisa de conferência.');
        if(item.points&&batch.expires_ms>now())coinWallet.grant(order.user_id,{sourceId:`legacy-reward-refund:${order.reference}:${item.id}`,amountAtoms:atomsFromRewardPoints(item.points,100),origin:'legacy_reward_refund',createdAt:batch.created_ms,expiresAt:batch.expires_ms,termsVersion:order.terms_version});
      }else db.prepare('UPDATE city_reward_batches SET remaining=MIN(points,remaining+?) WHERE id=? AND user_id=?').run(item.points,item.id,order.user_id);
    }
    db.prepare('UPDATE city_reward_orders SET debited=0 WHERE reference=?').run(order.reference);
  }
  const settle=db.transaction((reference,payment)=>{
    ...
    if(unified()){
      ...
      const cents=matched?BigInt(matched[1])*100n+BigInt((matched[2]||'').padEnd(2,'0')):-1n;
      if(!/^\d{1,30}$/.test(String(payment.id||''))||!/^\d{1,30}$/.test(String(expectedCollectorId||''))||String(payment.collector_id)!==String(expectedCollectorId)||payment.live_mode!==true||payment.currency_id!=='BRL'||cents!==BigInt(order.pay_cents))throw Error('Pagamento não corresponde ao pedido.');
    }
    ...
    if(status==='approved'){
      if(!order.debited&&order.points){
        ...
        const allocations=debit(order.user_id,order.points,`${reference}:recovery:${payment.id}`,order.kind);db.prepare('UPDATE city_reward_orders SET allocations=?,debited=1 WHERE reference=?').run(JSON.stringify(allocations),reference);
      }
      if(unified()){
        const current=db.prepare('SELECT allocations FROM city_reward_orders WHERE reference=?').get(reference);
        for(const item of JSON.parse(current.allocations))if(item.coinRequestId)coinWallet.settle(order.user_id,item.coinRequestId,{actualAtoms:item.amountAtoms,receiptId:`mercadopago:${payment.id}`});
      }
  ...
  const grantGame=db.transaction((id,points,sourceKey)=>{
    const config=settings();if(!config.enabled||!Number.isSafeInteger(points)||points<1)return 0;
    const time=now(),grant=Math.min(points,dailyAllowance(id,time).remaining);if(!grant)return 0;
    const result=db.prepare('INSERT OR IGNORE INTO city_reward_batches(user_id,source_key,points,remaining,created_ms,expires_ms) VALUES(?,?,?,?,?,?)').run(id,sourceKey,grant,unified()?0:grant,time,time+60*DAY);
    if(!result.changes)return 0;
    if(unified())coinWallet.grant(id,{sourceId:`reward:${sourceKey}`,amountAtoms:atomsFromRewardPoints(grant,100),origin:'validated_task',createdAt:time,expiresAt:time+60*DAY,termsVersion:VITRINE_COINS_POLICY.version});
    return grant;
  });
  ...
  app.post('/api/rewards/checkout',requireUser,sameOriginOnly,async(req,res)=>{
    ...
    if(!paymentConfigured())return res.status(503).json({error:'Pagamento temporariamente indisponível.'});
    let order;
    try{order=db.transaction(()=>{
      ...
      const reference='cityperk_'+randomUUID(),allocations=debit(req.user.id,q.points,reference,p.kind);
  ...
  async function refreshOrder(req,res,admin=false){
    ...
    if(!searchPayments||!paymentConfigured())return res.status(503).json({error:'Consulta de pagamento temporariamente indisponível.'});
  ...
  app.put('/api/admin/rewards',requireAdmin,sameOriginOnly,(req,res)=>{const rate=Number(req.body?.coinsPerReal),limit=Number(req.body?.dailyLimit);if(unified()&&rate!==100)return res.status(409).json({error:'A equivalência unificada está fixada em 9,6 Vitrine Coins por real. Os pontos internos deste adaptador permanecem 100 por real.',code:'coin_conversion_fixed'});if(!Number.isSafeInteger(rate)||rate<1||rate>10000||!Number.isSafeInteger(limit)||limit<1||limit>1000)return res.status(400).json({error:'Informe 1 a 10.000 moedas por real e limite diário de 1 a 1.000.'});db.transaction(()=>{db.prepare('UPDATE city_reward_settings SET coins_per_real=?,daily_limit=?,enabled=? WHERE id=1').run(rate,limit,req.body?.enabled===false?0:1);db.prepare('INSERT INTO city_reward_audit(admin_id,coins_per_real,daily_limit,enabled,created_ms) VALUES(?,?,?,?,?)').run(req.user.id,rate,limit,req.body?.enabled===false?0:1,now());})();return res.json({ok:true});});
