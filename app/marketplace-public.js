...
export function renderPublicStorePage({ store, products = [], siteUrl, productFallback = DEFAULT_PRODUCT_FALLBACK }) {
  ...
  const productCards = safeProducts.map(product => `<article class="card">
      ...
      <a class="button" href="${escapeHtml(product.buyUrl || product.path)}"${product.buyUrl?' target="_blank" rel="noopener sponsored"':''}>${product.buyUrl?'Comprar':'Ver produto'}</a></div></article>`).join('');
