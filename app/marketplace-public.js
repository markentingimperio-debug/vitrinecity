...
export function renderPublicStorePage({ store, products = [], siteUrl, productFallback = DEFAULT_PRODUCT_FALLBACK }) {
  ...
  const safeProducts = products.map(product => ({
    ...
    image: product.image_url || productFallback
  }));
  const channels = [
  ...
  return `<!doctype html><html lang="pt-BR"><head>
    ...
    <section class="hero"><img src="${escapeHtml(heroPath)}" onerror="this.onerror=null;this.src='${escapeHtml(productFallback)}'" alt="Fachada ou marca da loja ${escapeHtml(store.business_name)}">
