...
export function createSpatialTelemetryTracker({now=Date.now,bucketMs=300_000,retentionBuckets=12}={}){
  ...
  const newCounters=()=>({total:0,byEvent:Object.fromEntries(SPATIAL_TELEMETRY_EVENTS.map(v=>[v,0])),byDistrict:Object.fromEntries(SPATIAL_PRESENCE_DISTRICTS.map(v=>[v,0])),byProfile:{LITE:0,STANDARD:0,ULTRA:0,UNKNOWN:0},byFps:{poor:0,constrained:0,good:0,excellent:0,unknown:0},byTarget:{none:0,store:0,product:0,profile:0,course:0,service:0,campaign:0,content:0,restaurant:0,district:0,portal:0,business:0}});
  const prune=time=>{
    const min=bucketStart(time)-size*(keep-1);
    for(const key of buckets.keys())if(key<min)buckets.delete(key);
  };
  const record=input=>{
    ...
    bucket.total++;bucket.byEvent[normalized.event]++;bucket.byDistrict[normalized.district]++;bucket.byProfile[normalized.profile]++;bucket.byFps[normalized.fpsBucket]++;bucket.byTarget[normalized.targetType]++;
