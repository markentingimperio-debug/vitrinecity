...
test('TikTok uses the production millisecond expiry and WhatsApp reflects actual receipt evidence without rewriting legacy rows',()=>{const {db,service}=fixture();db.prepare('UPDATE tiktok_oauth_account SET expires_at=?').run(1788021123640);db.exec("CREATE TABLE whatsapp_qr_schedules(status TEXT,confirmation_state TEXT DEFAULT '',claimed_at INTEGER,provider_message_id TEXT);INSERT INTO whatsapp_qr_schedules(status,provider_message_id) VALUES('sent','ACK1'),('sent',NULL),('pending',NULL),('pending',NULL);");const before=db.prepare('SELECT * FROM whatsapp_qr_schedules').all(),connections=service.snapshot().connections;assert.match(connections.find(c=>c.id==='tiktok').reason,/expirado/);const wa=connections.find(c=>c.id==='whatsapp');assert.equal(wa.status,'partial');assert.match(wa.reason,/1 envios aceitos pelo serviço e 2 pendentes/);assert.match(wa.reason,/1 sem confirmação/);assert.equal(wa.canPublish,false);assert.deepEqual(db.prepare('SELECT * FROM whatsapp_qr_schedules').all(),before);});
