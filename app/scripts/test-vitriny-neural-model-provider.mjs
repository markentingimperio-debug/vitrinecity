...
const server=http.createServer(async(req,res)=>{
  ...
  res.end(JSON.stringify({model:'mock-qwen',choices:[{message:{content:'Análise concluída sem alterar produção. Recomendo testes antes do merge.'}}],usage:{prompt_tokens:10,completion_tokens:12}}));
});
...
try{
  ...
  console.log(JSON.stringify({ok:true,model:result.output.model,usage:result.output.usage}));
