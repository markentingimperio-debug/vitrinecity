...
try{
  for(const path of ['/cidade','/cidade.html','/vitriny-mini-fazenda.html','/vitriny-mini-fazenda','/%76itriny-games.html','/v/br/go/silvania']){assert.equal(memberPage(path),true);const r=await fetch(base+path,{redirect:'manual'});assert.equal(r.status,302,path);assert.ok(r.headers.get('location').startsWith('/entrar-cidade.html?returnTo='));}
  for(const path of ['/','/loja','/produto/12/adubo','/vitriny-multiverse-worlds.html'])assert.equal((await fetch(base+path)).status,200,path);
...
console.log(JSON.stringify({ok:true,membership:'page gates, public commerce, account isolation, server time, atomic progress'}));
