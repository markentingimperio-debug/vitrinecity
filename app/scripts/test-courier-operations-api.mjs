...
try{
  ...
  const jobId=Number(db.prepare("INSERT INTO local_delivery_jobs(order_reference,distance_meters,fee_cents,platform_cents,courier_cents,status) VALUES ('shop-courier',900,500,50,450,'available')").run().lastInsertRowid);db.close();
  ...
  response=await request(`/api/courier/jobs/${jobId}`,{method:'PATCH',headers:{cookie},body:JSON.stringify({action:'accept'})});assert.equal(response.status,200);
