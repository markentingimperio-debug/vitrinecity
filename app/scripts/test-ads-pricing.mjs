...
try{
  ...
  assert.deepEqual(quote,{dailyCredits:288,durationDays:30,dailyBudgetCents:3000,requestedNetUnits:864000,amountCents:105883,feeCents:15882,mediaCents:90001,grossCreditUnits:1016477,managementCreditUnits:152472,netCreditUnits:864005,creditsPerReal:9.6,managementRatePercent:15,validityDays:60});
  ...
  response=await request('/api/credits/checkout',{method:'POST',headers:{cookie},body:JSON.stringify({...checkoutBase,campaignChannel:'rede-inventada',targetAudience:'Pessoas interessadas no produto',reachKm:10})});assert.equal(response.status,400);assert.match((await response.json()).error,/canal/i);
  const walletHtml=readFileSync(new URL('../public/carteira.html',import.meta.url),'utf8');for(const id of ['campaignChannel','targetAudience','targetCity','reachKm','startsOn','creativeTitle','destinationUrl'])assert.match(walletHtml,new RegExp(`id=["']${id}["']`));
  ...
  assert.match(protectedReport.channelLabel,/VitrineCity/);assert.match(protectedReport.statusLabel,/anúncio interno/);
  const adminDb=new Database(path.join(dataDir,'vitrinecity.db'));adminDb.prepare("UPDATE users SET is_admin=1 WHERE id=?").run(userId);adminDb.prepare("UPDATE ad_campaigns SET status='funded',campaign_channel='meta',placement='external',external_platform_status='pending_setup' WHERE id=?").run(campaignId);adminDb.close();
  response=await request(`/api/admin/ad-campaigns/${campaignId}`,{method:'PATCH',headers:{cookie},body:JSON.stringify({action:'activate',notes:'Pronta'})});assert.equal(response.status,409);assert.match((await response.json()).error,/identificador/i);
  response=await request(`/api/admin/ad-campaigns/${campaignId}`,{method:'PATCH',headers:{cookie},body:JSON.stringify({action:'activate',notes:'Pronta',externalCampaignId:'meta-123'})});assert.equal(response.status,200);
  response=await request('/api/admin/ad-campaigns',{headers:{cookie}});const managed=(await response.json()).campaigns[0];assert.equal(managed.campaign_channel,'meta');assert.equal(managed.external_campaign_id,'meta-123');assert.equal(managed.external_platform_status,'connected');
