...
import { mountNeuralWorkspace } from '../public/neural-tasks.js';

const publicFile = name => new URL('../public/' + name, import.meta.url);
const html = readFileSync(publicFile('neural-tasks.html'), 'utf8');
const js = readFileSync(publicFile('neural-tasks.js'), 'utf8');
const css = readFileSync(publicFile('neural-tasks.css'), 'utf8');
execFileSync(process.execPath, ['--check', fileURLToPath(publicFile('neural-tasks.js'))]);
assert.match(html, /lang="pt-BR"/);
assert.match(html, /name="viewport"/);
assert.match(html, /<script type="module" src="\/neural-tasks\.js"><\/script>/);
...
assert.equal(store.elements.get('return-chat').href,'/neural-workspace.html?store=loja%2Fum');
