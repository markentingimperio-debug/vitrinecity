...
test('missing API key never uses Studio or network', async () => {
  ...
  for(const state of [service.status(),await service.check()]){
    assertKlingReadiness(state);assert.equal(state.stage,'credentials_missing');assert.equal(state.checkedAt,null);
...
test('outer success cannot override missing or failed inner account code',async()=>{
  for(const code of [1001,undefined,null,false,'0']){
    const service=createKlingReadiness({env:{KLING_API_KEY:'fake'},now:()=>NOW,fetchImpl:async()=>response({code:0,data:{code,resource_pack_subscribe_infos:[]}})});
