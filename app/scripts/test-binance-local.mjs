...
assert.match(server, /\/api\/admin\/integrations\/binance-trading/);
assert.match(server, /realEnabled: false, realLocked: true/);
assert.match(admin, /Binance · Executor local/);
assert.match(admin, /CONECTADO · SOMENTE LEITURA/);
assert.match(admin, /REAL BLOQUEADO/);
assert.match(admin, /DESLIGAR DEMO/);
...
assert.match(demo, /Robô demo desligado pelo painel administrativo/);
