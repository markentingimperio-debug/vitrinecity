...
assert.ok(merged.slice(0,8).every(item=>item.category===officialCategory),'Related platform results precede every external phrase');
assert.equal(merged.filter(item=>item.type==='web').length,2,'External suggestions fill remaining space');
...
const localFirst=mergeSuggestions([[{label:'adubação',type:'web'},{label:'plantas na internet',type:'web'}],[{label:'Curso de plantas',type:'content'},{label:'ADUBACAO',type:'content'},{label:'Adubo para plantas',type:'product'}]]);
assert.deepEqual(localFirst.map(item=>item.label),['Curso de plantas','ADUBACAO','Adubo para plantas','plantas na internet'],'Local courses, articles and products win duplicate labels even without accents');
...
const homeScript=fs.readFileSync(new URL('../public/vitriny-home.js',import.meta.url),'utf8');
...
assert.ok(home.includes(`action="${toCleanPublicHref('/pesquisar.html')}"`),'The native home form still submits to the search page');
assert.equal(toLegacyPublicPath('/pesquisar'),'/pesquisar.html');
assert.match(home,/id="home-search-suggestions"/);
assert.match(home,/<script[^>]+src="\/vitriny-home\.js(?:\?[^"]*)?"[^>]*defer/);
assert.match(homeScript,/import\('\/search-autocomplete\.js'\)/);
assert.match(homeScript,/query:search\.elements\.q/);
assert.match(homeScript,/suggestions:document\.getElementById\('home-search-suggestions'\)/);
for(const source of [homeScript,search])assert.match(source,/setupSearchAutocomplete/);
...
console.log('Autocomplete: relevant internal sources before web phrases, independent partial updates, deduplication, bounds, errors, cancellation and both page integrations verified.');
