...
try{
  ...
  response=await request(`/api/store-portal/${store}/marketplace?token=${encodeURIComponent(token)}`);assert.equal(response.status,200);const panel=await response.json();assert.equal(panel.orders.length,2);assert.equal(panel.returns[0].status,'received');assert.equal(panel.orders.find(order=>order.reference==='seller-order-ship').payoutStatus,'scheduled');
