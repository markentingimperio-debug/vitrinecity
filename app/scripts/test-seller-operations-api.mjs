...
try{
  ...
  response=await request(`/api/store-portal/${store}/marketplace?token=${encodeURIComponent(token)}`,{headers:sellerHeaders});assert.equal(response.status,200);const panel=await response.json();assert.equal(panel.orders.length,2);assert.equal(panel.returns[0].status,'received');const shipped=panel.orders.find(order=>order.reference==='seller-order-ship');assert.equal(shipped.payoutStatus,'scheduled');assert.equal(shipped.payoutCents,8750);assert.equal(shipped.reconciliation_status,'pending');assert.equal(panel.sellerProfile.complianceStatus,'verified');assert.deepEqual(panel.paymentSplit,{mode:'central_manual',configured:true,account:null});assert.deepEqual(panel.payoutPolicy,{weeklyDay:'tuesday',advanceFeePercent:3.5});assert.deepEqual(panel.manualPayouts,[]);assert.deepEqual(panel.fees,{percent:10,fixedCents:200,returnProvisionPerOrderCents:50});
