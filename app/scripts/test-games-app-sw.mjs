...
const ORIGIN='https://vitrinecity.test',CURRENT='vitrinecity-games-v2-share';
function harness(){
...
test('install caches public puzzles and exact static shell, never API, farm, account or model assets',async()=>{
  ...
  assert.ok(paths.length<=25,'small fixed shell including three public sharing dependencies');
...
test('offline puzzles preserve the same shell while farm returns an honest online-only page',async()=>{
  ...
  for(const pathname of ['/public-share.js','/public-share.css','/vitriny-public-routes.js'])assert.equal(await(await h.request(pathname+'?v=20260913-1',{mode:'cors'})).text(),pathname);
});
...
test('activation removes only older Games caches and leaves commerce and browser data untouched',async()=>{
  const h=harness();for(const name of ['vitrinecity-games-v0','vitrinecity-games-v1',CURRENT,'vitrinecity-shell-v99','customer-data'])await h.caches.open(name);
  await h.lifecycle('activate');assert.deepEqual(h.deleted,['vitrinecity-games-v0','vitrinecity-games-v1']);assert.equal(h.claims,1);
