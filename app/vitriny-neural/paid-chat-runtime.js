 ...
 */
export function createPaidChatRuntime({db,env=process.env,config:inputConfig,wallet:injectedWallet,artifacts,fetchImpl=globalThis.fetch,now=Date.now,pollIntervalMs=1000,authorizeScope=()=>true,reviewedKnowledgeProvider=null,liveEcosystemProvider=null}={}){
  ...
  function prepare(scope,{requestId,conversationId,kind,message,context={},referenceImage=null}={}){
    ...
    if(kind==='chat'){
      ...
      input=enrichPaidChatInput(input,{question:message,at:time,reviewedKnowledgeProvider,liveEcosystemProvider});
