...
function text(value,max,min=0,multiline=false){const raw=String(value??''),v=raw.trim(),controls=multiline?/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/:/[\u0000-\u001f\u007f]/;if(v.length<min||v.length>max||controls.test(raw))throw new Error('Entrada de marketing inválida.');return v;}
...
export function createGrowthSkill({neural=null}={}){
  return {
    ...
    async execute({input,invoke,context={}}){
      ...
      const request={
        objective:text(input.objective,2000,3,true),
        businessContext:text(input.businessContext||'',5000,0,true),
      ...
      if(neural?.ingest)try{neural.ingest({type:`skill.${capability}.${isIncompleteResponse(result.output)?'incomplete':'completed'}`,source:'vitriny-neural',entityType:'skill',entityId:'growth.optimizer',priority:2,payload:{provider:result.provider,objective:request.objective.slice(0,180),learningContext:context.learningContext||{}}});}catch{}
