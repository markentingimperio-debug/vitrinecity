...
function text(value,max,min=0,multiline=false){const raw=String(value??''),v=raw.trim(),controls=multiline?/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/:/[\u0000-\u001f\u007f]/;if(v.length<min||v.length>max||controls.test(raw))throw new Error('Entrada de pesquisa inválida.');return v;}
...
export function createResearchSkill({neural=null}={}){
  return {
    ...
    async execute({input,invoke,context={}}){
      ...
      const request={
        question:text(input.question,5000,3,true),
      ...
      if(neural?.ingest)try{neural.ingest({type:`skill.${capability}.${isIncompleteResponse(result.output)?'incomplete':'completed'}`,source:'vitriny-neural',entityType:'skill',entityId:'research.supervised',priority:2,payload:{provider:result.provider,maxSources:request.maxSources,learningContext:context.learningContext||{}}});}catch{}
