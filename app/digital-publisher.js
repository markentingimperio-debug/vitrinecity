...
export function setupDigitalPublisher({
  ...
  app.patch("/api/admin/books/:id",requireAdmin,sameOriginOnly,(req,res)=>{const item=book(req.params.id);if(!item)return res.status(404).json({error:"Livro não encontrado."});const body=req.body||{},title=String(body.title??item.title).trim().slice(0,180),category=bookCategory(body.category??item.category),summary=String(body.summary??item.summary).trim().slice(0,2000),audience=String(body.audience??item.audience).trim().slice(0,800),cover=String(body.coverUrl??item.cover_url).trim().slice(0,1000);if(title.length<3||summary.length<40)return res.status(400).json({error:"Informe título e resumo completos."});db.prepare("UPDATE digital_books SET title=?,category=?,summary=?,audience=?,cover_url=?,status=CASE WHEN status='published' THEN 'review' ELSE status END,published_at=CASE WHEN status='published' THEN NULL ELSE published_at END,updated_at=CURRENT_TIMESTAMP WHERE id=?").run(title,category,summary,audience,cover,item.id);db.prepare("UPDATE managed_courses SET status='paused',updated_at=CURRENT_TIMESTAMP WHERE slug=?").run(`livro-${item.slug}`);res.json({ok:true,item:book(item.id)})});
  app.patch("/api/admin/books/:id/chapters/:position",requireAdmin,sameOriginOnly,(req,res)=>{const item=book(req.params.id),position=Number(req.params.position);if(!item)return res.status(404).json({error:"Livro não encontrado."});const current=db.prepare("SELECT * FROM digital_book_chapters WHERE book_id=? AND position=?").get(item.id,position);if(!current)return res.status(404).json({error:"Capítulo não encontrado."});const title=String(req.body?.title??current.title).trim().slice(0,180),content=String(req.body?.content??current.content).trim().slice(0,100000),words=content?content.split(/\s+/).length:0;if(title.length<3||words<300)return res.status(400).json({error:"O capítulo precisa de título e pelo menos 300 palavras."});db.prepare("UPDATE digital_book_chapters SET title=?,content=?,status='approved',updated_at=CURRENT_TIMESTAMP WHERE id=?").run(title,content,current.id);const total=db.prepare("SELECT COALESCE(SUM(LENGTH(content)-LENGTH(REPLACE(content,' ',''))+1),0) words FROM digital_book_chapters WHERE book_id=?").get(item.id);db.prepare("UPDATE digital_books SET word_count=?,page_count=?,status='review',published_at=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=?").run(total.words,Math.floor(total.words/300),item.id);db.prepare("UPDATE managed_courses SET status='paused',updated_at=CURRENT_TIMESTAMP WHERE slug=?").run(`livro-${item.slug}`);res.json({ok:true,words,totalWords:total.words,pages:Math.floor(total.words/300)})});
  app.post(
    ...
    (req, res) => {
      ...
      db.prepare(
        ...
        item.audience,
        item.cover_url,
        `/api/my-courses/livro-${item.slug}/book`,
      );
