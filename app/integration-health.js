...
const AI_OPERATIONS = {
  ...
  kling_studio_account: 'Kling Studio · conexão e créditos',
...
export function classifyIntegrationFailure(error = {}) {
  ...
  if (/kling_studio_auth_unavailable/.test(message)) return 'authentication';
  if (/kling_studio_(?:not_configured|home_(?:invalid|missing)|model_invalid)/.test(message)) return 'not_configured';
  if (/kling_studio_(?:insufficient_credits|credits_insufficient|credits_unavailable)/.test(message)) return 'billing';
