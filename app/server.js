...
app.get(['/produto/:id', '/produto/:id/:slug'], (req, res) => {
  ...
  const importedReviewCount=db.prepare(`SELECT COUNT(*) count FROM marketplace_product_reviews r JOIN marketplace_review_sources rs ON rs.review_id=r.id WHERE r.product_id=? AND r.status='published'`).get(product.id).count;
  ...
  res.set('Cache-Control', 'public,max-age=60').send(`<!doctype html><html lang="pt-BR"><head>
    ...
    <section class="reviews"><h2>Avaliações de clientes</h2>${importedReviewCount?`<p class="description">Inclui ${importedReviewCount} ${importedReviewCount===1?'avaliação':'avaliações'} importada${importedReviewCount===1?'':'s'} da Shopee. A origem está indicada em cada comentário.</p>`:''}${reviews.length?`<div class="review-grid">${reviews.map(review=>`<article class="review"><div class="rating">${'★'.repeat(review.rating)}${'☆'.repeat(5-review.rating)}</div><h3>${escapeHtml(review.title||'Avaliação do produto')}</h3><p>${escapeHtml(review.body)}</p><small>${escapeHtml(review.author_name)} · ${new Date(`${review.created_at}Z`).toLocaleDateString('pt-BR')}</small>${renderImportedReviewSource(review,escapeHtml)}${review.verified_purchase&&!review.source?'<div class="verified">✓ Compra verificada</div>':''}</article>`).join('')}</div>`:'<p class="description">Este produto ainda não recebeu avaliações. As avaliações publicadas aparecerão aqui.</p>'}</section>
    </main>
