...
export function createYouTubeOAuth({db,encrypt,decrypt,siteUrl,expectedChannelId=YOUTUBE_PRAYER_CHANNEL,fetchImpl=fetch,now=Date.now}){
  ...
  if(site.protocol!=='https:'||site.username||site.password||site.port||expectedChannelId!==YOUTUBE_PRAYER_CHANNEL)throw failure('youtube_configuration_invalid');
  const redirectUri=site.origin+API+'/callback';let refreshing=null;
  ...
  function status(){
    ...
    return {configured:Boolean(c.client_id&&c.client_secret_encrypted),connected,requiresConnection:!connected,channelId:expectedChannelId,channelTitle:connected?a.channel_title:null,revision:connected?a.revision:null,
...
export function setupYouTubeOAuth({app,requireAdmin,sameOriginOnly,getSessionKey,...deps}){
  ...
  const safeError=(_error,res)=>res.status(409).json({error:'Não foi possível concluir a conexão do YouTube. Confira o aplicativo, as permissões e o canal autorizado.'});
