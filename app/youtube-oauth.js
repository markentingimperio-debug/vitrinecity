...
export const YOUTUBE_LIVE_CHAT_SCOPES=Object.freeze(['https://www.googleapis.com/auth/youtube.readonly','https://www.googleapis.com/auth/youtube.force-ssl']);
const API='/api/admin/prayer-sharing/youtube',CHAT_API='/api/admin/live-studio/youtube-chat/oauth',TOKEN_URL='https://oauth2.googleapis.com/token';
...
export function createYouTubeOAuth({db,encrypt,decrypt,siteUrl,expectedChannelId=YOUTUBE_PRAYER_CHANNEL,fetchImpl=fetch,now=Date.now,intent='upload'}){
  if(!['upload','live-chat'].includes(intent))throw failure('youtube_configuration_invalid');
  const prefix=intent==='live-chat'?'youtube_live_chat_':'youtube_upload_',base=intent==='live-chat'?CHAT_API:API;
  const requiredScopes=intent==='live-chat'?YOUTUBE_LIVE_CHAT_SCOPES:YOUTUBE_UPLOAD_SCOPES;
  const hasScopes=value=>requiredScopes.every(scope=>scopes(value).includes(scope));
  const prepare=sql=>db.prepare(sql.replaceAll('youtube_upload_',prefix)),exec=sql=>db.exec(sql.replaceAll('youtube_upload_',prefix));
  ...
  const redirectUri=enabled?site.origin+base+'/callback':null;let refreshing=null;
  const requireSecureOrigin=()=>{if(!enabled)throw failure('youtube_https_origin_required');};
  exec(`CREATE TABLE IF NOT EXISTS youtube_upload_app(id INTEGER PRIMARY KEY CHECK(id=1),client_id TEXT NOT NULL DEFAULT '',client_secret_encrypted TEXT NOT NULL DEFAULT '',revision INTEGER NOT NULL DEFAULT 0);
  ...
  db.transaction(()=>{const columns=prepare('PRAGMA table_info(youtube_upload_account)').all();for(const name of ['refresh_failures','refresh_next_at'])if(!columns.some(c=>c.name===name))exec(`ALTER TABLE youtube_upload_account ADD COLUMN ${name} INTEGER NOT NULL DEFAULT 0`);}).immediate();
  const config=()=>prepare('SELECT * FROM youtube_upload_app WHERE id=1').get();
  const account=()=>prepare('SELECT * FROM youtube_upload_account WHERE id=1').get();
  function status(){
    if(!enabled)return {enabled:false,configured:false,connected:false,requiresConnection:false,channelId:expectedChannelId,channelTitle:null,revision:null,redirectUri:null,scopes:[...requiredScopes],status:'unconfigured',publicUploadVerified:false,refreshRetryAt:null,configurationError:'youtube_https_origin_required',detail:'YouTube desativado neste ambiente. Configure um endereço público HTTPS para autorizar o canal.'};
    ...
    return {intent,enabled:true,configured:Boolean(c.client_id&&c.client_secret_encrypted),connected,requiresConnection:!connected,channelId:expectedChannelId,channelTitle:connected?a.channel_title:null,revision:connected?a.revision:null,
      redirectUri,scopes:[...requiredScopes],status:a?.status||'not_connected',publicUploadVerified:false,refreshRetryAt:connected&&a.refresh_next_at>now()?a.refresh_next_at:null,
      detail:connected&&a.refresh_next_at>now()?'O Google está temporariamente indisponível. A renovação será conferida novamente após o intervalo de espera.':connected?(intent==='live-chat'?'Canal autorizado para ler e responder ao chat. Ative a conexão somente na sessão de live em andamento.':'Canal autorizado. Um upload recebido como privado não confirma publicação pública.'):'Autorize o canal para permitir o envio. Conectar não ativa a rotina.'};
  }
  function configure({clientId,clientSecret}){
    ...
    db.transaction(()=>{prepare('UPDATE youtube_upload_app SET client_id=?,client_secret_encrypted=?,revision=revision+1 WHERE id=1').run(clientId,protectedSecret);exec('DELETE FROM youtube_upload_account; DELETE FROM youtube_upload_oauth_states;');}).immediate();return status();
  }
  function begin({adminId,sessionKey}){
    ...
    db.transaction(()=>{
      prepare('UPDATE youtube_upload_app SET revision=revision+1 WHERE id=1').run();
      exec("DELETE FROM youtube_upload_oauth_states; UPDATE youtube_upload_account SET status='reconnecting';");
      prepare('INSERT INTO youtube_upload_oauth_states VALUES(?,?,?,?,?,?)').run(hash(state),adminId,hash(sessionKey),encrypt(verifier),c.revision+1,now()+600000);
    }).immediate();
    const url=new URL('https://accounts.google.com/o/oauth2/v2/auth');
    url.search=new URLSearchParams({client_id:c.client_id,redirect_uri:redirectUri,response_type:'code',scope:requiredScopes.join(' '),access_type:'offline',prompt:'consent',state,
  ...
  async function complete({state,code,adminId,sessionKey}){
    ...
    const attempt=db.transaction(()=>{
      const row=prepare('SELECT * FROM youtube_upload_oauth_states WHERE state_hash=? AND admin_id=? AND session_hash=? AND expires_at>?').get(hash(state),adminId,hash(sessionKey),now());
      if(!row||row.revision!==config().revision)throw failure('youtube_oauth_state_invalid');
      prepare('DELETE FROM youtube_upload_oauth_states WHERE state_hash=?').run(hash(state));return row;
    }).immediate();
    const c=config();
    ...
    db.transaction(()=>{
      ...
      prepare(`INSERT INTO youtube_upload_account(id,channel_id,channel_title,access_encrypted,refresh_encrypted,expires_at,scope,revision,app_revision,status) VALUES(1,?,?,?,?,?,?,?,?,'connected')
  ...
  function disconnect(){if(!enabled)return status();db.transaction(()=>{prepare('UPDATE youtube_upload_app SET revision=revision+1 WHERE id=1').run();exec('DELETE FROM youtube_upload_account; DELETE FROM youtube_upload_oauth_states;');}).immediate();return {...status(),detail:'Conexão removida desta plataforma. Você também pode revogar o acesso nas permissões da Conta Google.'};}
  async function renew(){
    ...
    if(a.refresh_owner){if(a.refresh_until<=now())prepare("UPDATE youtube_upload_account SET status='needs_review' WHERE revision=? AND refresh_owner=?").run(a.revision,a.refresh_owner);throw failure('youtube_refresh_pending_review');}
    const owner=randomUUID(),c=config();
    if(!prepare("UPDATE youtube_upload_account SET refresh_owner=?,refresh_until=? WHERE id=1 AND revision=? AND refresh_owner='' AND status='connected'").run(owner,now()+45000,a.revision).changes)throw failure('youtube_refresh_busy');
    try{
      ...
      const saved=prepare("UPDATE youtube_upload_account SET access_encrypted=?,refresh_encrypted=?,expires_at=?,scope=?,refresh_owner='',refresh_until=0,refresh_failures=0,refresh_next_at=0 WHERE id=1 AND revision=? AND app_revision=? AND refresh_owner=? AND status='connected' AND refresh_until>?")
      ...
      if(error.code==='youtube_oauth_temporary'){
        ...
        const saved=prepare("UPDATE youtube_upload_account SET status=?,refresh_failures=?,refresh_next_at=?,refresh_owner='',refresh_until=0 WHERE id=1 AND revision=? AND app_revision=? AND refresh_owner=? AND status='connected'")
      ...
      prepare("UPDATE youtube_upload_account SET status='needs_review' WHERE id=1 AND revision=? AND refresh_owner=?").run(a.revision,owner);throw failure(error.code==='youtube_reconnect_required'?error.code:'youtube_refresh_pending_review');
    }
  }
...
export function createYouTubeLiveChatOAuth(deps){return createYouTubeOAuth({...deps,intent:'live-chat'});}

export function setupYouTubeOAuth({app,requireAdmin,sameOriginOnly,getSessionKey,intent='upload',...deps}){
  const service=createYouTubeOAuth({...deps,intent}),base=intent==='live-chat'?CHAT_API:API;
  const target=intent==='live-chat'?'/admin-live.html?youtubeChat=':'/admin-youtube.html?youtube=';
  ...
  app.get(base+'/status',requireAdmin,(_req,res)=>res.set('Cache-Control','no-store').json(service.status()));
  app.post(base+'/app',requireAdmin,sameOriginOnly,(req,res)=>{try{res.set('Cache-Control','no-store').json(service.configure(req.body||{}));}catch(e){safeError(e,res);}});
  app.post(base+'/connect',requireAdmin,sameOriginOnly,(req,res)=>{try{res.set('Cache-Control','no-store').json(service.begin(identity(req)));}catch(e){safeError(e,res);}});
  app.get(base+'/callback',requireAdmin,async(req,res)=>{
    try{await service.complete({...identity(req),state:req.query.state,code:req.query.code});res.redirect(303,target+'connected');}
    catch{res.redirect(303,target+'needs_review');}
  });
  app.post(base+'/disconnect',requireAdmin,sameOriginOnly,(_req,res)=>res.set('Cache-Control','no-store').json(service.disconnect()));
  return service;
}

export function setupYouTubeLiveChatOAuth(deps){return setupYouTubeOAuth({...deps,intent:'live-chat'});}
