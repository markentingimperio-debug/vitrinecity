#!/usr/bin/env python3
"""Narrow LIA deployment: exact current app image, no .env mutation or DB restore."""
import argparse
...
def run(args,cwd=None,timeout=180,input=None):
    try:p=subprocess.run(args,cwd=cwd,input=input,text=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE,timeout=timeout)
    except subprocess.TimeoutExpired:raise RuntimeError('Comando excedeu o limite; saida privada nao exibida.')
    if p.returncode:raise RuntimeError('Falhou: '+' '.join(map(str,args[:3]))+'. Saida omitida para proteger configuracoes privadas.')
...
def compose_for(info,root):
    labels=info['Config'].get('Labels') or {};project=labels.get('com.docker.compose.project','')
    if not project or labels.get('com.docker.compose.service')!='app':raise RuntimeError('Container nao e o servico app de um projeto Compose.')
    if Path(labels.get('com.docker.compose.project.working_dir','')).resolve()!=root:raise RuntimeError('Diretorio Compose divergente.')
    files=labels.get('com.docker.compose.project.config_files','').split(',')
    if not files or any(not f or not Path(f).is_file() for f in files):raise RuntimeError('Arquivos Compose ativos nao foram encontrados.')
...
def data_mount(info):
    ...
    if len(rows)!=1 or rows[0]['Type'] not in ('volume','bind'):raise RuntimeError('Banco precisa estar em armazenamento persistente conhecido.')
    return {k:rows[0][k] for k in ('Type','Source','Destination')}
def node(cid,code,*args):return run(['docker','exec','-w','/app',cid,'node','--input-type=module','-e',code,*args])
def assert_idle(cid):
...
const has=t=>!!db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name=?").get(t);let busy=0;
...
console.log(busy);db.close();"""
    if node(cid,code)!='0':raise RuntimeError('Ha tarefas da LIA em andamento. Nenhuma foi cancelada. Publique depois que terminarem.')
def current_id(compose,root):
    ids=run(compose+['ps','-a','-q','app'],cwd=root).splitlines()
...
const r=await fetch('http://127.0.0.1:3000/api/health',{signal:AbortSignal.timeout(4000)});if(!r.ok)process.exit(1);
if(process.argv[1]){const d=fs.readFileSync('/app/vitriny-neural/chat-engine.js');if(createHash('sha256').update(d).digest('hex')!==process.argv[1]||process.env.LIA_LOCAL_FIRST_ADMIN!=='1')process.exit(1);}console.log('ok');"""
...
def restore(folder):
    ...
    for f in m['files']:
        dest=root/f['path']
        if dest.is_symlink() or not dest.is_file() or sha(dest.read_bytes()) not in (f['before'],f['after']):raise RuntimeError('Arquivo alterado depois do instalador. Restauracao bloqueada: '+f['path'])
    check_config(m['compose'],{'Config':{'Labels':{'com.docker.compose.config-hash':m['configHash']}}},root)
    ids=run(m['compose']+['ps','-a','-q','app'],cwd=root).splitlines()
    if len(ids)>1:raise RuntimeError('Mais de um app; restauracao bloqueada.')
    if ids:
        live=inspect(ids[0])
        if data_mount(live)!=m['dataMount'] or live['Image'] not in (m['oldImage'],m['newImage']):raise RuntimeError('Volume/imagem divergente; restauracao bloqueada.')
    for f in m['files']:atomic(root/f['path'],(folder/'files'/f['path']).read_bytes(),f['mode'])
    run(['docker','tag',m['oldImage'],m['oldTag']]);run(m['compose']+['up','-d','--no-deps','--no-build','--pull','never','app'],cwd=root)
    restored=inspect(wait_health(m['compose'],root))
    if restored['Image']!=m['oldImage'] or data_mount(restored)!=m['dataMount']:raise RuntimeError('Restauracao requer verificacao de imagem/volume.')
...
def install(root,cid,check_only=False):
    ...
    if any(m['Destination']=='/app' or m['Destination'].startswith('/app/') for m in info.get('Mounts',[])):raise RuntimeError('Codigo montado diretamente no app. Este instalador nao altera codigo em processo vivo.')
    origin=run(['git','remote','get-url','origin'],cwd=root).lower().rstrip('/')
    if not origin.endswith(('markentingimperio-debug/vitrinecity','markentingimperio-debug/vitrinecity.git')):raise RuntimeError('Repositorio diferente da Vitrine City.')
    engine=root/RELATIVE;dockerfile=root/'app/Dockerfile'
    if not engine.is_file() or not dockerfile.is_file() or engine.is_symlink() or dockerfile.is_symlink():raise RuntimeError('Arquivos esperados ausentes ou links simbolicos.')
    if MARKER in engine.read_text():
        if healthy(cid,sha(engine.read_bytes())):print('ESTA ATUALIZACAO JA ESTA PUBLICADA. Nenhuma nova mudanca.');return
    ...
    if env.get('LIA_LOCAL_FIRST_ADMIN') not in (None,'1'):raise RuntimeError('LIA_LOCAL_FIRST_ADMIN ja tem uma escolha diferente. Configuracao preservada.')
    if not env.get('KLING_API_KEY') or not (env.get('DEEPSEEK_API_KEY') or env.get('OPENAI_API_KEY')):raise RuntimeError('Chaves Kling/texto nao encontradas no ambiente ativo. Nenhuma chave foi mostrada ou criada.')
    ...
    if len(re.findall(r'^FROM\s',s,re.M|re.I))!=1 or 'LIA_LOCAL_FIRST_ADMIN' in s:raise RuntimeError('Dockerfile diferente do contrato esperado; nenhuma alteracao.')
    ...
    with tempfile.TemporaryDirectory(prefix='lia-reviewed-') as tmp:
        tmp=Path(tmp);(tmp/'chat-engine.js').write_bytes(patched);(tmp/'chat-engine.js').chmod(0o644);tmp.chmod(0o755)
        print('Executando testes isolados na imagem atual, sem APIs pagas...')
        report=run(['docker','run','--rm','--network','none','--read-only','--cap-drop','ALL','--security-opt','no-new-privileges','--tmpfs','/tmp:rw,nosuid,nodev,size=128m','-v',str(tmp/'chat-engine.js')+':/app/vitriny-neural/chat-engine.js:ro','-v',str(HERE/'test.mjs')+':/app/scripts/test-lia-preserve-kling.mjs:ro','--entrypoint','node',info['Image'],'--test','/app/scripts/test-lia-preserve-kling.mjs'],timeout=180)
        print('Testes do roteamento: APROVADOS.')
        if check_only:print('VERIFICACAO CONCLUIDA. Nada publicado.');return
        stamp=dt.datetime.now(dt.timezone.utc).strftime('%Y%m%d%H%M%S');folder=Path('/var/backups')/('vitrinecity-lia-kling-'+stamp);folder.mkdir(parents=True,mode=0o700)
        print('Backup: '+str(folder));old_tag=info['Config']['Image']
        if old_tag.startswith('sha256:') or '@sha256:' in old_tag:raise RuntimeError('Imagem sem tag substituivel; parei sem publicar.')
        base_tag='vitrinecity-lia-before:'+stamp;new_tag='vitrinecity-lia-reviewed:'+stamp;run(['docker','tag',info['Image'],base_tag])
        ...
        run(['docker','build','--network=none','--pull=false','-t',new_tag,str(tmp)],timeout=300);new_image=inspect(new_tag)['Id']
        ...
        (folder/'test-result.txt').write_text(report);snapshot_name='.lia-backup-'+stamp+'.sqlite'
        ...
        remote=node(cid,backup_code,snapshot_name);run(['docker','cp',cid+':'+remote,str(folder/'database.sqlite')],timeout=300);os.chmod(folder/'database.sqlite',0o600)
        node(cid,"import fs from 'node:fs';fs.unlinkSync(process.argv[1]);",remote)
        manifest={'root':str(root),'compose':compose,'oldImage':info['Image'],'oldTag':old_tag,'newImage':new_image,'dataMount':mount,'configHash':info['Config']['Labels']['com.docker.compose.config-hash'],'files':[{'path':RELATIVE,'before':sha(engine.read_bytes()),'after':sha(patched),'mode':engine.stat().st_mode&0o777},{'path':'app/Dockerfile','before':sha(d),'after':sha(patched_d),'mode':dockerfile.stat().st_mode&0o777}]}
        (folder/'manifest.json').write_text(json.dumps(manifest,indent=2));assert_idle(cid);check_config(compose,info,root)
        if current_id(compose,root)!=cid:raise RuntimeError('Container mudou durante a verificacao. Nada publicado.')
        if sha(engine.read_bytes())!=manifest['files'][0]['before'] or sha(dockerfile.read_bytes())!=manifest['files'][1]['before']:raise RuntimeError('Codigo mudou durante a verificacao. Nada publicado.')
        try:
            atomic(engine,patched,manifest['files'][0]['mode']);atomic(dockerfile,patched_d,manifest['files'][1]['mode']);run(['docker','tag',new_image,old_tag])
            print('Publicando somente o app; .env, Kling e volumes mantidos...');run(compose+['up','-d','--no-deps','--no-build','--pull','never','app'],cwd=root)
            live=wait_health(compose,root,sha(patched));live_info=inspect(live)
            if live_info['Image']!=new_image or data_mount(live_info)!=mount:raise RuntimeError('Imagem/volume final nao correspondem ao esperado.')
