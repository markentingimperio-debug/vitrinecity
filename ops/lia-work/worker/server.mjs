...
async function gitSnapshot(cwd){
  const [status,stat,names,untracked]=await Promise.all([
    runFixed('git',['status','--short','--untracked-files=all'],cwd),
    runFixed('git',['diff','--stat'],cwd),
    runFixed('git',['diff','HEAD','--name-only','--diff-filter=ACMR'],cwd),
    runFixed('git',['ls-files','--others','--exclude-standard'],cwd),
  ]);
  const changedFiles=[...new Set([names.stdout,untracked.stdout].flatMap(output=>output.trim().split(/\r?\n/).filter(Boolean)))].slice(0,200);
  return{status:status.stdout.trim(),diffStat:stat.stdout.trim(),changedFiles};
}
