...
// An SSE response can finish while the SDK starts its next request. Serialize
// writes so those two requests never rename the same temporary ledger file.
let persistQueue=Promise.resolve();
function persist(){
  const next=persistQueue.then(async()=>{
    const tmp=`${LEDGER_FILE}.tmp-${process.pid}-${randomUUID()}`;
    try{
