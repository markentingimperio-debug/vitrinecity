...
function validMoney(value){const scaled=value*1e6;return typeof value==='number'&&Number.isFinite(scaled)&&scaled>0&&Number.isSafeInteger(Math.round(scaled))&&Math.abs(Math.round(scaled)-scaled)<1e-6;}
function validWorkspace(value){return typeof value==='string'&&/^[a-zA-Z0-9][a-zA-Z0-9._-]{0,63}$/.test(value);}
async function provisionSiteWorkspace(accountId){
  if(!Number.isSafeInteger(accountId)||accountId<1||accountId>999999999999999)throw Object.assign(new Error('invalid_account'),{status:400});
  const root=await fs.realpath(WORKSPACE_ROOT),name=`account-${accountId}`,target=path.join(root,name);
  const owner={accountId,kind:'site',version:1};
  try{
    const stat=await fs.lstat(target);
    if(!stat.isDirectory())throw Object.assign(new Error('workspace_conflict'),{status:409});
    const real=await fs.realpath(target);
    if(!real.startsWith(root+path.sep))throw Object.assign(new Error('workspace_conflict'),{status:409});
    const saved=JSON.parse(await fs.readFile(path.join(real,'.lia-owner.json'),'utf8'));
    if(saved.accountId!==accountId||saved.kind!=='site'||saved.version!==1)throw Object.assign(new Error('workspace_conflict'),{status:409});
  ...
  catch(error){if(error?.code==='EEXIST')throw Object.assign(new Error('workspace_busy'),{status:409});throw error;}
  const temp=path.join(root,`.${name}-${randomUUID()}`);
  try{
    try{await fs.stat(target);throw Object.assign(new Error('workspace_busy'),{status:409});}catch(error){if(error?.code!=='ENOENT')throw error;}
    await fs.mkdir(temp,{mode:0o750});
    await fs.writeFile(path.join(temp,'index.html'),'<!doctype html>\n<html lang="pt-BR"><meta charset="utf-8"><title>Meu site</title><h1>Meu site</h1></html>\n',{mode:0o640});
    await fs.writeFile(path.join(temp,'README.md'),'# Site da conta\n\nRascunho privado para tarefas da Lia. Publicação requer uma etapa separada.\n',{mode:0o640});
    await fs.writeFile(path.join(temp,'.lia-owner.json'),JSON.stringify(owner)+'\n',{mode:0o600});
    await execFile('git',['init','-q','-b','main'],{cwd:temp,timeout:15000});
    await execFile('git',['add','index.html','README.md','.lia-owner.json'],{cwd:temp,timeout:15000});
    await execFile('git',['-c','user.name=Lia','-c','user.email=lia@vitrinecity.invalid','commit','-qm','Create private site workspace'],{cwd:temp,timeout:15000});
...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(req.method==='POST'&&url.pathname==='/v1/workspaces/provision'){
      const body=await readJson(req);
      if(Object.keys(body).sort().join(',')!=='accountId,kind'||body.kind!=='site')return send(res,400,{error:'invalid_workspace_request'});
      const workspace=await provisionSiteWorkspace(Number(body.accountId));
      return send(res,workspace.created?201:200,{ok:true,workspace});
    }
    const fileMatch=url.pathname.match(/^\/v1\/workspaces\/(account-[1-9]\d{0,14})\/file$/);
    if(req.method==='GET'&&fileMatch){
      const relative=String(url.searchParams.get('path')||'');
      if(!/^[A-Za-z0-9][A-Za-z0-9._/-]{0,179}$/.test(relative)||relative.split('/').some(part=>!part||part==='.'||part==='..'||part.startsWith('.'))||
        !/\.(?:html|css|js|json|md|svg|txt)$/i.test(relative))return send(res,400,{error:'invalid_file_path'});
      const root=await fs.realpath(WORKSPACE_ROOT);
      let workspace;try{workspace=await fs.realpath(path.join(root,fileMatch[1]));}catch(error){if(error?.code==='ENOENT')return send(res,404,{error:'workspace_not_found'});throw error;}
      if(!workspace.startsWith(root+path.sep))return send(res,403,{error:'workspace_escape_blocked'});
      let file;try{file=await fs.realpath(path.resolve(workspace,relative));}catch(error){if(error?.code==='ENOENT')return send(res,404,{error:'file_not_found'});throw error;}
      if(!file.startsWith(workspace+path.sep))return send(res,403,{error:'file_escape_blocked'});
      const stat=await fs.stat(file);
      if(!stat.isFile()||stat.size>2*1024*1024)return send(res,404,{error:'file_unavailable'});
      const body=await fs.readFile(file);
      res.writeHead(200,{'content-type':'application/octet-stream','content-length':body.length,'cache-control':'private,no-store',
        'x-content-type-options':'nosniff','content-disposition':`attachment; filename="${path.basename(file).replace(/[^A-Za-z0-9._-]/g,'_')}"`});
      res.end(body);return;
    }
