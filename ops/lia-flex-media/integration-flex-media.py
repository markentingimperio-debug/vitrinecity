...
def run_case(audio_enabled):
 ...
 m=mod('helper',REPO/'ops/lia-preserve-kling/deploy-lia-v92.py');u=mod('updater',HERE/'update-flex-media-v96.py');f=mod('flex',HERE/'build-flex-media-v96.py');b=mod('builder',HERE/'build-reference-media-v95.py');a=mod('audio',HERE/'build-native-audio-v94.py')
 ...
 for name in b.MANIFEST:
  p=ctx/name.removeprefix('app/');p.write_bytes(b.patch(name,p.read_bytes()))
 (ctx/'public/neural-reference-media.js').write_bytes((REPO/'app/public/neural-reference-media.js').read_bytes())
 if audio_enabled:
  for name,(before,_) in a.MANIFEST.items():
   p=ctx/name.removeprefix('app/');p.parent.mkdir(parents=True,exist_ok=True);p.write_bytes(a.patch(name,p.read_bytes() if before else None))
 ...
 env={'SITE_URL':'https://vitrinecity.com','LIA_CHAT_OPERATIONS_ENABLED':'true','LIA_OPERATIONS_TOKEN':'synthetic-not-a-credential',u.CONFIG_KEY:json.dumps({'enabled':True,'fx':{'usdToBrl':'5'},'kling':{'videoUsdPerSecond':'0.084','accountBinding':'synthetic',**({'nativeAudio720':u.TARIFF} if audio_enabled else {})}})}
 ...
 try:
  ...
  m.write_new(seed/'state.private.json',json.dumps({'kind':'LIA_REFERENCE_MEDIA_V95','phase':'DEPLOYED','newImage':original['Image'],'frozenHashes':{'after.private.json':m.sha(m.private_read(cfg))}}).encode())
  ...
  info,audio,done=u.inspect_ready(m,b,a,f);assert audio==audio_enabled and not done
  with patch.object(u,'quality',return_value={'fixtureOnly':True}):assert u.update(m,b,a,f,files,info,audio,'a'*40)==0
  newer,has_audio,done=u.inspect_ready(m,b,a,f);assert has_audio==audio_enabled and done and m.healthy(newer)
  assert m.environment(newer)==m.environment(original)
  work=Path(m.load(m.ROOT/'latest.private.json')['directory']);state=m.load(work/'state.private.json');assert state['phase']=='DEPLOYED'
  with sqlite3.connect(dbpath) as db:db.execute('INSERT INTO marker VALUES(2)')
  ...
  with sqlite3.connect(dbpath) as db:assert db.execute('SELECT count(*) FROM marker').fetchone()[0]==2
  results.append({'audioEnabled':audio_enabled,'tests':state['tests'],'rollbackPreservedNewDatabaseWrites':True})
  info,audio,done=u.inspect_ready(m,b,a,f)
  with patch.object(u,'quality',return_value={'fixtureOnly':True}),patch.object(m,'api_probe',side_effect=m.Blocked('SIMULATED_POST_SWAP_FAILURE')):
   assert u.update(m,b,a,f,files,info,audio,'a'*40)==2
  assert m.app()['Image']==original['Image'] and m.healthy(m.app()) and m.load(m.ROOT/'latest.private.json')==latest
  with sqlite3.connect(dbpath) as db:assert db.execute('SELECT count(*) FROM marker').fetchone()[0]==2
  print('FLEX_DOCKER_RESULT='+json.dumps({'cases':results,'automaticRollbackAfterFailure':True,'databaseRestored':False,'productionAccess':False,'providerCalls':0}),flush=True)
 finally:
...
if __name__=='__main__':
 for audio in (False,True):run_case(audio)
