#!/usr/bin/env python3
"""Flexible reference-media routing: v95-only pinned overlay, no configuration changes.
...
KIND='LIA_FLEX_MEDIA_V96'
PR=213
WORKFLOW='LIA flexible routing validation'
...
def load_helper():
    ...
    m.REQUIRED_CI=(*m.REQUIRED_CI,'LIA image routing hotfix tests','LIA native audio validation','LIA reference media validation',WORKFLOW)
    return m
...
def load_bundle(m):
    ...
    f=module('flex_builder',files['build-flex-media-v96.py'],root/'build-flex-media-v96.py')
    need(sha(files['neural-reference-media-v96.js'])==f.MANIFEST[f.MODULE][1],'MODULO_DIVERGENTE')
    return b,a,f,files
...
def quality(m,revision):
    ...
    need(pr.get('head',{}).get('repo',{}).get('full_name')==m.REPO and pr.get('head',{}).get('ref')=='fix/lia-flex-media-20260920' and
         pr.get('base',{}).get('ref')=='feat/lia-product-reference-20260919','BASE_OU_BRANCH_NAO_REVISADA')
...
def expected_files(m,b,a,f,audio=False,flex=False):
    values={**m.PAYLOAD,**{p:v[int(audio)] for p,v in a.MANIFEST.items()},
     **{p:v[1] for p,v in b.MANIFEST.items()},b.MODULE:b.MODULE_HASH}
    values.update({p:v[int(flex)] for p,v in f.MANIFEST.items()})
    return values
...
def inspect_ready(m,b,a,f):
    ...
    need(state.get('phase')=='DEPLOYED' and state.get('newImage')==info['Image']==latest.get('image') and
         state.get('kind') in ('LIA_REFERENCE_MEDIA_V95',KIND),'BASE_GERENCIADA_DIVERGENTE')
    ...
    actual=m.runtime_hashes(info['Id'],expected_files(m,b,a,f));matches=[]
    for audio in (False,True):
        for ref in (False,True):
            if actual==expected_files(m,b,a,f,audio,ref):matches.append((audio,ref))
...
def build_image(m,f,files,info,work):
    ...
    payload={p:f.patch(p,read(p),files['neural-reference-media-v96.js']) for p in f.MANIFEST}
    context=work/'context';context.mkdir(mode=0o700)
    for p,data in payload.items():m.write_new(context/p,data)
    base='vitrinecity-lia:flex-base-'+work.name;tag='vitrinecity-lia:flex-'+work.name
...
def reference_tests(m,tag,files,work,audio):
    paths=[]
    for name in ('test-reference-media.mjs','test-flex-media.mjs'):
        p=work/'tests'/name;m.write_new(p,files[name]);paths.extend(['--mount',f'type=bind,source={p},target=/app/scripts/{name},readonly'])
    ...
    cmd=['docker','run','--init','--rm','--name',name,'--pull','never','--network','none','--read-only','--no-healthcheck',
      ...
      *paths,'--entrypoint','/usr/bin/env',tag,
      '-i','PATH=/usr/local/bin:/usr/bin:/bin','HOME=/tmp','TMPDIR=/tmp','REFERENCE_AUDIO_ENABLED='+str(int(audio)),
      'node','--test','--test-reporter=tap','scripts/test-reference-media.mjs','scripts/test-flex-media.mjs']
    try:
        ...
        need(r.returncode==0 and counts==dict(tests=120,**{'pass':120},fail=0,cancelled=0,skipped=0,todo=0),'TESTES_DE_REFERENCIA_FALHARAM');return counts
    finally:
        try:m.command(['docker','rm','-f',name],timeout=20,allow_failure=True)
        except m.Blocked:pass

def update(m,b,a,f,files,info,had_audio,revision):
    quality(m,revision);guards(m,info,a)
    want_audio=had_audio;env=m.environment(info).copy()
    ...
    m.write_new(work/'update-flex-media-v96.py',Path(__file__).read_bytes())
    m.write_new(work/'before-inspect.private.json',json.dumps(info).encode())
    print('Construindo roteamento flexivel e testando sem credenciais/dados de producao.',flush=True)
    base,tag,image=build_image(m,f,files,info,work)
    ...
    check=copy.deepcopy(after);check['services']['app']['image']=before['services']['app']['image']
    need(check==before,'CONFIGURACAO_FORA_DA_IMAGEM')
    ...
    rollback=f'python3 {work}/update-flex-media-v96.py voltar --estado {work.name} --confirmar-troca'
    m.write_new(work/'rollback.sh',('#!/bin/sh\nset -eu\nexec '+rollback+'\n').encode());print('Retorno desta atualizacao: '+rollback,flush=True)
    changed=False
    try:
        current,audio,already=inspect_ready(m,b,a,f)
        ...
        m.verify_payload(active['Id'],expected_files(m,b,a,f,want_audio,True));m.api_probe(active['Id'])
        ...
        print('=== RELATORIO LIA: ROTEAMENTO FLEXIVEL ===')
        print(json.dumps({'status':'FLEX_MEDIA_ROUTING_ENABLED_E2E_PENDING','deployed':True,'appHealthy':True,
          'referenceImageToVideoEnabled':True,'referenceImageToImageEnabled':True,'nativeAudioEnabled':want_audio,
          'flexibleMediaRoutingEnabled':True,'ambiguityRequiresClarification':True,'sameEnvironment':True,'visualAnalysisEnabled':False,'operationsEnabled':True,'initEnabled':True,'sameDataMounts':True,
...
def main():
    ...
    p.add_argument('--revision',default='');p.add_argument('--estado',default='');p.add_argument('--confirmar-troca',action='store_true');args=p.parse_args()
    ...
    if args.mode=='plano':
        b,a,f,files=load_bundle(m);info,audio,already=inspect_ready(m,b,a,f)
        if not already:quality(m,args.revision);guards(m,info,a);m.find_backup()
        print(json.dumps({'status':'ALREADY_UPDATED' if already else 'READY_NOT_DEPLOYED','appHealthy':True,
          'deployedByThisCommand':False,'nativeAudioPreserved':audio,'paidTaskExecuted':False},indent=2));return 0
    ...
    try:
        ...
        b,a,f,files=load_bundle(m);info,audio,already=inspect_ready(m,b,a,f)
        if already:
            print('ALREADY_UPDATED: nenhuma troca executada.');return 0
        return update(m,b,a,f,files,info,audio,args.revision)
