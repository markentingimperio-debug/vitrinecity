...
MARKER=/opt/lia/data/vitrinecity-first-real-task-v6.attempted
...
[ ! -e "$MARKER" ] || { echo "PARADO: tentativa V6 ja foi iniciada. Marcador: $MARKER" >&2; exit 1; }
...
[ -z "$STATUS" ] || { echo 'PARADO: workspace precisa estar limpo antes da tarefa V6.' >&2; printf '%s\n' "$STATUS" >&2; exit 1; }
...
echo 'AF_NETLINK do Worker: ATIVO'
...
echo
echo '=== RESULTADO LIA V6 / VITRINE CITY ==='
...
if [ "$CURL_RC" -ne 0 ] || [ "$HTTP_CODE" != '200' ]; then
  echo 'TAREFA V6 NAO CONCLUIDA. Nao havera repeticao automatica.'
  exit 1
fi
