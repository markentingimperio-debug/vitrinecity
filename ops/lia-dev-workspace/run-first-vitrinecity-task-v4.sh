...
for cmd in curl jq git sudo systemctl sed grep stat date journalctl awk; do
...
MARKER=/opt/lia/data/vitrinecity-first-real-task-v4.attempted
...
[ ! -e "$MARKER" ] || { echo "PARADO: tentativa V4 ja foi iniciada. Marcador: $MARKER" >&2; exit 1; }
...
[ -z "$STATUS" ] || { echo 'PARADO: workspace precisa estar limpo antes da tarefa V4.' >&2; printf '%s\n' "$STATUS" >&2; exit 1; }
...
curl -fsS http://127.0.0.1:8787/health   | jq -e '.version=="2026-09-18-v4-stream" and .executionEnabled==false' >/dev/null   || { echo 'PARADO: gateway V4 nao confirmado.' >&2; exit 1; }

curl -fsS http://127.0.0.1:8791/health   | jq -e '.version=="2026-09-18-v4-stream" and .actualUsageAccounting==true and .ssePassthrough==true and .executionEnabled==false' >/dev/null   || { echo 'PARADO: broker V4 nao confirmado.' >&2; exit 1; }

curl -fsS http://127.0.0.1:8790/health   | jq -e '.version=="2026-09-18-v3-diagnostics" and .sdkLoaded==true and .executionEnabled==false' >/dev/null   || { echo 'PARADO: worker diagnostico V3 nao confirmado.' >&2; exit 1; }
...
INSTRUCTION='Trabalhe somente no workspace atual. Leia apenas app/web-story-cta.js e crie somente app/scripts/test-web-story-cta.mjs. Use node:test e node:assert/strict, sem dependencias. Nao altere nenhum arquivo existente. Cubra estes comportamentos: product e affiliate retornam "Ver oferta"; store "Visitar loja"; course "Ver curso"; service "Ver serviço"; city "Explorar cidade"; sourceKind funciona quando kind nao existe; recipe, group recipes, group receitas, portal receitas e category receitas retornam "Ver modo de preparo"; fonte desconhecida e objeto vazio retornam "Ler matéria completa". Depois rode exatamente node --test app/scripts/test-web-story-cta.mjs. Nao use rede. Nao execute npm install. Nao faça git add, commit, push ou deploy. Ao terminar, responda em no maximo 4 linhas com arquivo criado e resultado do teste.'
...
echo
echo '=== RESULTADO LIA V4 / VITRINE CITY ==='
...
if [ "$CURL_RC" -ne 0 ] || [ "$HTTP_CODE" != '200' ]; then
  echo
  echo 'TAREFA V4 NAO CONCLUIDA. Nao havera repeticao automatica.'
  exit 1
fi
...
printf '\nfinished_at=%s\nhttp=%s\n'   "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$HTTP_CODE" >>"$MARKER"

trap - EXIT
disable_all

echo
echo '=== TAREFA REAL V4 CONCLUIDA ==='
